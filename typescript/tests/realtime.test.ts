...
describe("DeliveryLog", () => {
  ...
  it("retains exactly its configured capacity and filters explicit stream sets", () => {
    ...
    expect(log.entries().map((entry) => entry.cursor.sequence)).toEqual([
      12, 13,
    ]);
    expect(
      log.recoverAfter({ epoch: "epoch-a", sequence: 11 }, new Set(["a"])),
    ).toEqual({
      kind: "replay",
    ...
    expect(
      log.recoverAfter({ epoch: "epoch-a", sequence: 11 }, new Set()),
    ).toEqual({
      kind: "replay",
      latestCursor: { epoch: "epoch-a", sequence: 13 },
      entries: [],
  ...
  it("fails before mutating the log when the delivery sequence is exhausted", () => {
    ...
    expect(() => log.append("a", 1, "payload")).toThrow(
    ...
    expect(
      log.recoverAfter({ epoch: "epoch-a", sequence: MAX_DELIVERY_SEQUENCE }),
    ).toEqual({
      kind: "replay",
  ...
  it("validates epoch, capacity, initial sequence, stream version, and cursors", () => {
    ...
    expect(() => new DeliveryLog({ epoch: "epoch-a", maxEntries: 0 })).toThrow(
      /maxEntries/u,
    );
    expect(
...
describe("reconnectDelayMs", () => {
  ...
  it("rejects fractional, negative, and inverted backoff inputs", () => {
    expect(() => reconnectDelayMs(-1, { baseMs: 1, maxMs: 2 })).toThrow(
      /attempt/u,
    );
    expect(() => reconnectDelayMs(1, { baseMs: 0.5, maxMs: 2 })).toThrow(
      /baseMs/u,
    );
