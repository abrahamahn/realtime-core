...
describe('reconnect state', () => {
  it('does not reset backoff merely because a transport opened', () => {
    ...
    const second = scheduleReconnectAttempt(open, { baseMs: 1_000, maxMs: 15_000 });
  ...
  it('resets only after stability and can enforce a retry floor', () => {
    ...
    expect(scheduleReconnectAttempt(raised, { baseMs: 1_000, maxMs: 15_000 }).delayMs).toBe(
      15_000,
    );
    expect(markReconnectStable(raised)).toEqual({ attempt: 0, status: 'stable' });
