...
import { createClientRecoveryState, reduceClientRecovery } from '../src/index.js';
...
describe('client recovery reducer', () => {
  ...
  it('keeps compatibility updates without cursors transport-neutral', () => {
    ...
    const decision = reduceClientRecovery(state, { kind: 'update', stream: 'legacy' });
    expect(decision.state).toBe(state);
    expect(decision.invalidation).toEqual({ kind: 'streams', streams: ['legacy'] });
  });
});
